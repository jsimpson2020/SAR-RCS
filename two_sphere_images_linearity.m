...
% Compute KM image for data from dielectric spheres. Then compute the
% modified KM image using the subtraction method. All units are with
...
% are those used to generate Fig. 6 in that manuscript.
%
...
nrel1 = 1.2;                            % refractive index of sphere 1
radius1 = 2*lambda0;                    % radius of sphere 1
center1 = [50*lambda0;-25*lambda0;0];   % center of sphere 1

nrel2 = 1.8;                            % refractive index of sphere 2
radius2 = 1.8*lambda0;                  % radius of sphere 2
center2 = [-30*lambda0;30*lambda0;0];   % center of sphere 2
...
search_x = linspace(-500/k0,500/k0,201);
...
plot(k0*center2(1)+k0*radius2*cos(s),k0*center2(2)+k0*radius2*sin(s),'w:',...
    'LineWidth',2);
hold off
% print('-depsc', 'two_sphere_images_linearity1.eps');
% print('-dpdf', 'two_sphere_images_linearity1.pdf');
...
plot(k0*center2(1)+k0*radius2*cos(s),k0*center2(2)+k0*radius2*sin(s),'w:',...
    'LineWidth',2);
hold off
% print('-depsc', 'two_sphere_images_linearity2.eps');
% print('-dpdf', 'two_sphere_images_linearity2.pdf');
