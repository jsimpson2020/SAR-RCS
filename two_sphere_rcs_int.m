...
% are those used to generate Fig. 9.
...
radius1 = 1.6*lambda0;                  % radius of sphere 1
center1 = [2*lambda0;-40*lambda0;0];    % center of sphere 1

nrel2 = 1.6;                            % refractive index of sphere 2
radius2 = 1.3*lambda0;                  % radius of sphere 2
center2 = [-2*lambda0;50*lambda0;0];    % center of sphere 2
...
fvec_m = zeros(k_num,array_size);

for p = 1:array_size

    BB_avg = BB(:,p);
    
    rhs = 4*pi*L^2*BB_avg.*(exp(-2i*k*L).');
    
    array_avg = array(:,p);
    
    A = zeros(k_num,k_num);
    A(1,1) = point(k(1),L,array_avg,peak1);
    A(1,k_num/2+1) = 1.5*point(k(1),L,array_avg,peak2);
    A(1,k_num/2+2) = -0.5*point(k(1),L,array_avg,peak2);
    for i = 2:k_num-1
        if abs(mod(i,2))<=1e-15
            A(i,i/2) = 0.5*point(k(i),L,array_avg,peak1);
            A(i,i/2+1) = 0.5*point(k(i),L,array_avg,peak1);
            A(i,i/2+k_num/2) = point(k(i),L,array_avg,peak2);
        else
            A(i,ceil(i/2)) = point(k(i),L,array_avg,peak1);
            A(i,floor(i/2)+k_num/2) = 0.5*point(k(i),L,array_avg,peak2);
            A(i,ceil(i/2)+k_num/2) = 0.5*point(k(i),L,array_avg,peak2);
        end
    end
    A(k_num,k_num/2-1) = -0.5*point(k(k_num),L,array_avg,peak1);
    A(k_num,k_num/2) = 1.5*point(k(k_num),L,array_avg,peak1);
    A(k_num,k_num) = point(k(k_num),L,array_avg,peak2);
    
    fvec_m(:,p) = A\rhs;

end

fvec = sum(fvec_m,2)/array_size;

RCS2_rec = 4*pi*abs(fvec(1:k_num/2)).^2;
RCS1_rec = 4*pi*abs(fvec(k_num/2+1:end)).^2;
...
k_even = zeros(1,k_num/2);
k_odd = zeros(1,k_num/2);
for i = 1:k_num/2
    k_even(i) = k(2*i);
    k_odd(i) = k(2*i-1);
end
...
freq = c*k/(2*pi)*1e-9;
freq_odd = c*k_odd/(2*pi)*1e-9;
freq_even = c*k_even/(2*pi)*1e-9;

figure('DefaultAxesFontSize',20)
plot(freq,RCS1_true,'k','LineWidth',2); hold on
plot(freq_odd,RCS1_rec,'ro','LineWidth',2);
xlim([min(freq),max(freq)]);
xlabel('frequency (GHz)', 'Interpreter','Latex');
ylabel('$\sigma_{RCS}$ (m$^2$)', 'Interpreter','Latex');
legend('true','recovered',...
    'Interpreter','Latex','Location','Best');
% print('-depsc', 'two_sphere_rcs_int1.eps');
% print('-dpdf', 'two_sphere_rcs_int1.pdf');

figure('DefaultAxesFontSize',20)
plot(freq,RCS2_true,'k','LineWidth',2); hold on
plot(freq_even,RCS2_rec,'ro','LineWidth',2);
xlim([min(freq),max(freq)]);
xlabel('frequency (GHz)', 'Interpreter','Latex');
ylabel('$\sigma_{RCS}$ (m$^2$)', 'Interpreter','Latex');
legend('true','recovered',...
    'Interpreter','Latex','Location','Best');
% print('-depsc', 'two_sphere_rcs_int2.eps');
% print('-dpdf', 'two_sphere_rcs_int2.pdf');

function f = point(k,L,array_center,peak)
    f = L^2/vecnorm(array_center-peak,2)^2*...
        exp(2i*k*(vecnorm(array_center-peak,2)-L));
end
